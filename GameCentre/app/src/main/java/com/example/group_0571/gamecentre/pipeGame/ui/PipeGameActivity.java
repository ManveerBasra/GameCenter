...
import com.example.group_0571.gamecentre.pipeGame.PipeGameActivityController;
...
public class PipeGameActivity extends AppCompatActivity implements GameActivityObserver {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pipe_game);
    ...
    private void setStateManager() {
        try {
            gameActivityController.setStateManager(saveManager.
                    loadFromTempFile(openFileInput(SaveManager.P_TEMP_SAVE_FILENAME)));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_pipe_game, menu);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.undo_button) {
            if (!gameActivityController.undoLastMove()) {
                Toast.makeText(PipeGameActivity.this, getString(R.string
                        .cannot_undo), Toast.LENGTH_SHORT).show();
            } else if (gameActivityController.getStateManager().isFinished()) {
                Toast.makeText(PipeGameActivity.this, getString(R.string.game_over),
    ...
    private void saveToTempFile() {
        try {
            saveManager.saveToTempFile(openFileOutput(SaveManager
                            .P_TEMP_SAVE_FILENAME, MODE_PRIVATE),
    ...
    private void saveToSaveFile() {
        ...
        try {
            fileMap = saveManager.loadFromSaveFileAllUsers(
                    openFileInput(SaveManager.P_SAVE_FILENAME));
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            fileMap = new HashMap<>();
        }
        try {
            saveManager.saveToSaveFile(openFileOutput
                    (SaveManager.P_SAVE_FILENAME, MODE_PRIVATE), fileMap, username, gameActivityController.getStateManager());
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    }
    ...
    private void saveToScoreboard(List<User> users) {
        try {
            saveManager.saveScoresToFile(openFileOutput
                            (ScoreboardActivity.SCORES_PATH, MODE_PRIVATE),
                    users);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
    ...
    private void moveToScoreboard() {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                startActivity(new Intent(PipeGameActivity.this,
    ...
    private void toastDelayed(final String message) {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(PipeGameActivity.this, message, Toast.LENGTH_SHORT).show();
            }
        }, 2000);
    }
    ...
    public void onGameFinished(List<User> users, boolean newHighScore) {
        Toast.makeText(PipeGameActivity.this, getString(R.string.you_win), Toast.LENGTH_SHORT).show();
